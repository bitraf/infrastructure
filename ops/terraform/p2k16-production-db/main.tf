...
module "p2k16-db" {
  ...
  postgres_password = data.terraform_remote_state.pdb.outputs.pdb-p2k16-production_vault.pdb_postgres_password
  host              = "p2k16-production.lan.bitraf.no"
  env               = "production"
}

module "terraform-output" {
  source = "../modules/terraform-output"
  name   = "p2k16-production-db"
